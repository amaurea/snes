.SECTION "BasicInit" SEMIFREE

	; This can't be in a subroutine because the stack isn't set up
	; before this
	.MACRO InitCPU
		sei         ; disable IRQ
...
.SECTION "InitMode1" SEMIFREE
	...
	InitMode1:
		...
		lda #$21
		sta $2105
		; Enable all three backgrounds and the sprites. Less confusing than
		; having them disabled by default
		lda #%00010111
		sta $212c
		; Tile map locations
		lda #$00  ; BG1 tile map at vram $0000. Len $400. 1x1 tilemaps
		sta $2107
		lda #$04  ; BG2 tile map at vram $0400. Len $400. 1x1 tilemaps
		sta $2108
		lda #$08  ; BG3 tile map at vram $0800. Len $400. 1x1 tilemaps
		sta $2109
		; Character data locations. $400 possible tiles. Each $10/$20/$40 bytes for 2/4/8 bpp,
		; so size of $4000/$8000/$10000 bytes, or $2000/$4000/$8000 words
		lda #$51  ; BG1 char at $1000, BG2 char at $5000. Each len $4000 words
		sta $210b
		lda #$09  ; BG3 char at $9000. Len $2000 words
		sta $210c
		rts
.ENDS
